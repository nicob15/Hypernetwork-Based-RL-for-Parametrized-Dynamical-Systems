...
class Meta_Embedding(nn.Module):
    ...
    def __init__(self, meta_dim, z_dim):
        ...
        super(Meta_Embedding, self).__init__()
...
class Hyper_Network(nn.Module):
    def __init__(self, meta_v_dim, base_v_dim, output_dim, h_dim):
...
# class Hyper_Network_3layers(nn.Module):
#     def __init__(self, meta_v_dim, base_v_dim, output_dim, h_dim):
#         super(Hyper_Network_3layers, self).__init__()
#
#         dynamic_layer = h_dim
#         z_dim = 1024
#         self.output_dim = output_dim
...
#         # Policy net
#         self.layer1 = Head(z_dim, base_v_dim, dynamic_layer, sttdev=0.05)
#         self.layer2 = Head(z_dim, dynamic_layer, dynamic_layer, sttdev=0.05)
#         self.last_layer = Head(z_dim, dynamic_layer, output_dim, sttdev=0.008)
#
#     def forward(self, meta_v, base_v, debug=None):
#         # produce dynamic weights
#         z = self.hyper(meta_v)
#         w1, b1, s1 = self.layer1(z)
#         w12, b12, s12 = self.layer2(z)
#         w2, b2, s2 = self.last_layer(z)
#
#         # dynamic network pass
#         out = F.relu(torch.bmm(w1, base_v.unsqueeze(2)) * s1 + b1)
#         out = F.relu(torch.bmm(w12, out) * s12 + b12)
#         out = torch.bmm(w2, out) * s2 + b2
#
#         return out.view(-1, self.output_dim)
#         # # dynamic network pass
...
class Hyper_Policy(nn.Module):
    # Hyper net that create weights from the parameter for a net that estimates the policy pi(S)
    def __init__(self, task_dim, state_dim, action_dim, h_dim, max_action):
        ...
        self.hyper = Hyper_Network(meta_v_dim=task_dim, base_v_dim=state_dim, output_dim=action_dim, h_dim=h_dim)

    def forward(self, state, task):

        z = self.hyper.get_z(task)
        action = self.hyper.dynamic(z, state)

        return self.max_action * torch.tanh(action)

# class Hyper_Policy_3layers(nn.Module):
#     # Hyper net that create weights from the parameter for a net that estimates the policy pi(S)
#     def __init__(self, task_dim, state_dim, action_dim, h_dim, max_action):
#         super(Hyper_Policy_3layers, self).__init__()
#         self.task_dim = task_dim
#         self.state_dim = state_dim
#         self.action_dim = action_dim
#         self.num_hidden = h_dim
#         self.max_action = max_action
#
#         self.hyper = Hyper_Network_3layers(meta_v_dim=task_dim, base_v_dim=state_dim, output_dim=action_dim, h_dim=h_dim)
#
#     def forward(self, state, task):
#
#         z = self.hyper.get_z(task)
