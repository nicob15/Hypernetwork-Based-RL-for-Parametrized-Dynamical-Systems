...
class Actor(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, max_action, hidden_size=256):
...
class IQN(nn.Module):
    ...
    def forward(self, input, action, num_tau=32):
        ...
        x = (x.unsqueeze(1) * cos_x).view(batch_size * num_tau,
                                          self.layer_size)  # batch_size*num_tau, self.cos_layer_out
...
class DeepActor(nn.Module):
    ...
    def forward(self, state):
        ...
        return self.max_action * torch.tanh(self.fc5(x))
...
class DeepIQN(nn.Module):
    ...
    def forward(self, input, action, num_tau=32):
        ...
        #       tau2              a2
        #       ..]]              ..]]
        action = action.repeat(num_tau, 1).reshape(num_tau, batch_size * self.action_size).transpose(0, 1).reshape(
            batch_size * num_tau, self.action_size)
        state = input.repeat(num_tau, 1).reshape(num_tau, batch_size * self.input_shape).transpose(0, 1).reshape(
            batch_size * num_tau, self.input_shape)
