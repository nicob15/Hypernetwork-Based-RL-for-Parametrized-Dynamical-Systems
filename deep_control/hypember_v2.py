...
class SunriseAgent:
    def __init__(
        ...
        if actor_net_cls is None:
            actor_net_cls = nets.HyperStochasticActorE
        if critic_net_cls is None:
            critic_net_cls = nets.HyperCriticE
        ...
        self.actors = actor_net_cls(obs_space_size, act_space_size, log_std_low, log_std_high,
                                    random_vector_size=random_vector_size, dist_impl="pyd").to(self.device)
        self.critics = critic_net_cls(obs_space_size, act_space_size, random_vector_size=random_vector_size).to(self.device)
    ...
    def to(self, dev):
        self.device = dev
        self.critics = self.critics.to(dev)
        self.actors = self.actors.to(dev)
    ...
    # --------------------------------------------------
    def save(self, path):
        actor_path = os.path.join(path, f"actor.pt")
        critic_path = os.path.join(path, f"critic.pt")
        torch.save(self.actors.state_dict(), actor_path)
        torch.save(self.critics.state_dict(), critic_path)

    def load(self, path):
        actor_path = os.path.join(path, f"actor.pt")
        critic_path = os.path.join(path, f"critic.pt")
        self.actors.load_state_dict(torch.load(actor_path, weights_only=False))
        self.critics.load_state_dict(torch.load(critic_path, weights_only=False))
    ...
    def forward(self, state, from_cpu=True):
        ...
        with torch.no_grad():
            act = torch.stack(
                [self.actors.forward(state).mean for _ in range(self.ensemble_size)], dim=0
            ).mean(0)
        self.train()
        if from_cpu:
    ...
    def sample_action(self, state, from_cpu=True):
        ...
        with torch.no_grad():
            ...
            #     [self.actors.forward(state_t).sample().squeeze(0) for _ in range(self.ensemble_size)],
            #     dim=0,
            # )
            s_t = state_t.unsqueeze(0).repeat(self.ensemble_size, 1, 1)
            act_candidates = self.actors.forward(s_t).sample().squeeze(1)

            # valutiamo ogni azione con ogni critic: shape (n_critics, n_actions, 1)
            # q_vals = torch.stack(
            #     [
            #         self.critics(state_t.repeat(len(act_candidates), 1), act_candidates)
            #         for _ in range(self.ensemble_size)
            #     ],
            #     dim=0,
            # )
            q_vals = self.critics(state_t.repeat(len(act_candidates), 1), act_candidates)
...
def learn_sunrise(
    ...
    with torch.no_grad():
        ...
        #     [target_agent.critics(state_batch, action_batch) for _ in range(ensemble_size)], dim=0
        # ).std(0)
        target_q_std = target_agent.critics(state_batch.repeat(ensemble_size, 1),
                                            action_batch.repeat(ensemble_size, 1)).reshape(ensemble_size, batch_size, -1).std(0)
    ...
    ns_batch = next_state_batch.unsqueeze(0).repeat(ensemble_size, 1, 1)
    action_dist = agent.actors(ns_batch)
    actions = action_dist.rsample()
    logp_a = action_dist.log_prob(actions).sum(-1, keepdim=True)
    target_q_s = target_agent.critics(ns_batch, actions)

    s_batch = state_batch.unsqueeze(0).repeat(ensemble_size, 1, 1)
    a_batch = action_batch.unsqueeze(0).repeat(ensemble_size, 1, 1)
    agent_critic_pred = agent.critics(s_batch, a_batch)
    for i in range(ensemble_size):
        with torch.no_grad():
            # compute TD target value for this critic
            td_target = reward_batch + gamma * (1.0 - done_batch) * (
                    target_q_s[i] - (log_alphas[i].exp() * logp_a[i])
            )
        ...
        total_abs_td_error += torch.abs(td_target - agent_critic_pred[i])
    ...
    if critic_clip:
        torch.nn.utils.clip_grad_norm_(
            chain(*(agent.critics.parameters())), critic_clip
        )
    ...
    actor_loss = 0.0
    s_batch = state_batch.unsqueeze(0).repeat(ensemble_size, 1, 1)
    dist = agent.actors(s_batch)
    agent_actions = dist.rsample()
    logp_a = dist.log_prob(actions).sum(-1, keepdim=True)

    critic_pred = agent.critics(s_batch, agent_actions)
    for i in range(ensemble_size):
        actor_loss += -(critic_pred[i] - (log_alphas[i].exp().detach() * logp_a[i])).mean()

        # update alpha_i
        alpha_loss = (-log_alphas[i].exp() * (logp_a[i] + target_entropy).detach()).mean()
