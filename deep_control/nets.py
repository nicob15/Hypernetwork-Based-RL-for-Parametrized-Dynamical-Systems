...
from deep_control.nets_blocks import Head, Meta_Embedding, Hyper_Network
...
class HyperStochasticActorE(nn.Module):
    def __init__(self, state_space_size, act_space_size, log_std_low=-10, log_std_high=2, hidden_size=1024,
                 dist_impl="pyd", random_vector_size=1024):
        super().__init__()
        ...
        self.hyper = Hyper_Network(meta_v_dim=state_space_size + random_vector_size, base_v_dim=state_space_size,
                                   output_dim=2*act_space_size, h_dim=hidden_size, random_vector_size=random_vector_size)

    def forward(self, state):
        """
        state: tensor shape (batch_size, state_space_size)
        output: distribuzione sulle azioni in [-1, 1]
        """
        out = self.hyper(state, state)

        # controllo di sicurezza: l'output deve avere 2 * act_space_size colonne
        expected_dim = 2 * self.act_space_size
        if out.dim() == 3:

            mu, log_std = out.chunk(2, dim=2)
            if self.dist_impl == "pyd":
                # clamp log_std nel range [log_std_low, log_std_high]
            ...
            if out.shape[1] != expected_dim:
                raise ValueError(
                    f"StochasticActor.fc3 output dim mismatch: got {out.shape[1]}, "
                    f"expected {expected_dim} = 2 * act_space_size (act_space_size={self.act_space_size})."
                )
...
class HyperStochasticActor(nn.Module):
    def __init__(self, state_space_size, act_space_size, log_std_low=-10, log_std_high=2, hidden_size=256, dist_impl="pyd"):
        super().__init__()
        assert dist_impl in ["pyd", "beta"]
        ...
        self.hyper = Hyper_Network(meta_v_dim=state_space_size, base_v_dim=state_space_size, output_dim=2*act_space_size, h_dim=hidden_size)
...
class StochasticActor(nn.Module):
    def __init__(
        ...
        # deve produrre 2 * act_space_size (mu e log_std)
...
class HyperActor(nn.Module):
    def __init__(self, state_space_size, act_space_size, hidden_size=256):
        super().__init__()

        self.state_space_size = state_space_size
        self.act_space_size = act_space_size

        self.hyper = Hyper_Network(meta_v_dim=state_space_size, base_v_dim=state_space_size, output_dim=act_space_size, h_dim=hidden_size)

    def forward(self, state):
        """
        state: tensor shape (batch_size, state_space_size)
        output: azioni in [-1, 1]
        """
        out = self.hyper(state, state)

        return torch.tanh(out)

class HyperCriticE(nn.Module):
    def __init__(self, state_space_size, act_space_size, hidden_size=256, random_vector_size=1024):
        super().__init__()

        dynamic_layer = hidden_size
        z_dim = 1024

        self.hyper = Meta_Embedding(state_space_size + random_vector_size, z_dim)

        # Q function net
        self.layer1 = Head(z_dim, state_space_size + act_space_size, dynamic_layer, sttdev=0.05)
    ...
    def forward(self, state, action, debug=None):
        ...
        if state.dim() == 3:
            rnd_z = torch.rand(state.shape[0], state.shape[1], self.random_vector_size).to(state.device)
            z = self.hyper(torch.cat([state, rnd_z], dim=2))
            w1, b1, s1 = self.layer1(z)
            w2, b2, s2 = self.last_layer(z)

            base_v = torch.cat([state, action], dim=2)

            # dynamic network pass
            out = F.relu(torch.bmm(w1, base_v.reshape(-1, base_v.shape[2]).unsqueeze(2)) * s1 + b1)
            out = torch.bmm(w2, out) * s2 + b2

            return out.view(state.shape[0], state.shape[1], 1)
        else:
            rnd_z = torch.rand(state.shape[0], self.random_vector_size).to(state.device)
            z = self.hyper(torch.cat([state, rnd_z], dim=1))
            w1, b1, s1 = self.layer1(z)
            w2, b2, s2 = self.last_layer(z)

            base_v = torch.cat([state, action], dim=1)
...
class HyperCritic(nn.Module):
    def __init__(self, state_space_size, act_space_size, hidden_size=256):
        ...
        self.hyper = Meta_Embedding(state_space_size, z_dim)
