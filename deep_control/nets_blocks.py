...
# class Hyper_Network(nn.Module):
#     def __init__(self, meta_v_dim, base_v_dim, output_dim, h_dim, random_vector_size):
#         super(Hyper_Network, self).__init__()
#
#         dynamic_layer = h_dim
#         z_dim = 1024
...
#
#         #self.rnd_z = torch.rand(5, 256, self.random_vector_size)
#
#     def forward(self, meta_v, base_v, debug=None):
#         # produce dynamic weights
#         if meta_v.dim() == 3:
#             rnd_z = torch.rand(meta_v.shape[0], meta_v.shape[1], self.random_vector_size).to(meta_v.device)
#             #rnd_z = self.rnd_z.to(meta_v.device)
#
#             z = self.hyper(torch.cat([meta_v, rnd_z], dim=2))
#             w1, b1, s1 = self.layer1(z)
#             w2, b2, s2 = self.last_layer(z)
#
#             # dynamic network pass
...
#             out = torch.bmm(w2, out) * s2 + b2
#
#             return out.view(meta_v.shape[0], meta_v.shape[1], self.output_dim)
#         else:
#             rnd_z = torch.rand(meta_v.shape[0], self.random_vector_size).to(meta_v.device)
#             z = self.hyper(torch.cat([meta_v, rnd_z], dim=1))
#             w1, b1, s1 = self.layer1(z)
#             w2, b2, s2 = self.last_layer(z)
#
#             # dynamic network pass
