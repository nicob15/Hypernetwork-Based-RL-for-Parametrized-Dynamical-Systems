...
class ReplayBufferStorage:
    ...
    def add(self, s, a, r, s_1, d):
        """
        Aggiunge uno o più transition:
        - s: stato (obs_dim,) o (batch, obs_dim)
        - a: azione (act_dim,) o (batch, act_dim)
        - r: reward (scalar, array, ecc.)
        - s_1: next_state
        - d: done (bool, int, array)
        """
        # ---- converto tutto in numpy se serve ----
        ...
        # ---- shape: stato ----
        # s shape finale: (batch, *obs_shape)
        if s.dim() == len(self.obs_shape):
            s = s.view(1, *self.obs_shape)
        elif s.dim() == len(self.obs_shape) + 1:
            # già batched
            pass
        else:
            raise ValueError(f"Unexpected state shape {s.shape}, obs_shape={self.obs_shape}")

        # ---- shape: next_state ----
        if s_1.dim() == len(self.obs_shape):
            s_1 = s_1.view(1, *self.obs_shape)
        elif s_1.dim() == len(self.obs_shape) + 1:
            pass
        else:
            raise ValueError(f"Unexpected next_state shape {s_1.shape}, obs_shape={self.obs_shape}")

        # ---- shape: azione ----
        if a.dim() == len(self.act_shape):
            a = a.view(1, *self.act_shape)
        elif a.dim() == len(self.act_shape) + 1:
            pass
        else:
            raise ValueError(f"Unexpected action shape {a.shape}, act_shape={self.act_shape}")

        # ---- reward e done ----
        r = torch.as_tensor(r, dtype=torch.float32).view(-1, 1)
        d = torch.as_tensor(d, dtype=self.done_stack.dtype).view(-1, 1)

        num_samples = s.shape[0]

        # Store at end of buffer. Wrap around if past end.
        R = np.arange(self._next_idx, self._next_idx + num_samples) % self.size
        self.s_stack[R] = s.to(self.s_dtype)
        self.action_stack[R] = a.float()
        self.reward_stack[R] = r
        self.s1_stack[R] = s_1.to(self.s_dtype)
        self.done_stack[R] = d
...
class ReplayBuffer:
    ...
    def _convert_dtype(self, dtype):
        if dtype in [int, np.uint8, torch.uint8]:
            ...
            raise ValueError(f"Unrecognized replay buffer dtype: {dtype}")
