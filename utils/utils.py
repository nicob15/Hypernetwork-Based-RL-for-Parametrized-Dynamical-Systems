...
class ReplayBuffer(object):
    ...
    def add(self, state, action, next_state, reward, done):
        self.state[self.ptr] = state.astype('float32')
        self.action[self.ptr] = action.astype('float32')
        self.next_state[self.ptr] = next_state.astype('float32')
...
class ReplayBufferD4PG:
    """Fixed-size buffer to store experience tuples."""

    def __init__(self, batch_size, n_step, device, seed, discount, parallel_env=1, max_size=int(1e6)):
...
class PrioritizedReplay(object):
    ...
    def __init__(self, batch_size, device, seed, discount=0.99, n_step=1, parallel_env=1, alpha=0.6,
                 beta_start=0.4, beta_frames=100000, max_size=int(1e6)):
        self.alpha = alpha
    ...
    def add(self, state, action, next_state, reward, done):
        ...
        max_prio = np.array(self.priorities,
                            dtype=float).max() if self.buffer else 1.0  # gives max priority if buffer is not empty else 1
